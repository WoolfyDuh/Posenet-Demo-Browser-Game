...
async function showSkeleton(poseArray){
    ...
    ellipse(poseArray.keypoints[0].position.x, poseArray.keypoints[0].position.y, nosePoint);
      nosePoint.draw(ctx);
    ellipse(poseArray.keypoints[9].position.x, poseArray.keypoints[9].position.y, leftWristPoint);
      leftWristPoint.draw(ctx);
    ellipse(poseArray.keypoints[10].position.x, poseArray.keypoints[10].position.y, rightWristPoint);
...
setInterval(() => {
  ...
  if(hasCollided && posenetOk ){
   ...
   function ellipse(x, y, point){
    point.position.dx = x;
    point.position.dy = y;
  }

  function resetCollisionBool(){
    hasCollided = false;
  }
